...
\begin{frame}{Scenario 1 Results (after bin-1 fix)}
  ...
  \begin{alertblock}{Current limitation}
    Deterministic dynamics + one decisive early burn = all three variants identical.
    The POMDP is not yet hard enough to differentiate centralized vs.\ decentralized.
    Next: stochastic T, finer bins, or reduced contact frequency.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Next Steps}
  \begin{enumerate}
    \item \textbf{Scenario 3 fix}: make sync optional --- encode ``communicate'' vs.\
          ``don't communicate'' as a choice using \texttt{sync\_actions} in RSSDA API
    \vspace{0.2em}
    \item \textbf{Scenario 2}: encode SC2's fixed (uncooperative) policy in T matrix:
          $T_\text{asym}[a,s,s'] = T_\text{full}[a_1 \cdot 3,\, s,\, s']$
    \vspace{0.2em}
    \item \textbf{Reward shaping}: continuous miss-distance penalty
          $R(d) = -1000 \cdot \exp(-d/d_\text{safe})$ instead of step thresholds
    \vspace{0.2em}
    \item \textbf{Trajectory deviation cost}: penalize total $\Delta v$ / orbital element change
    \vspace{0.2em}
    \item \textbf{Finer bins}: 8 bins --- split collision and high-risk zones
    \vspace{0.2em}
