...
Visualize the RS-SDA* policy as a (miss_bin, stage) heatmap.

For each starting (miss_bin, stage), re-solves RS-SDA* with a point-mass
initial belief on that state and reads the stage-0 action from the policy.
...
from spacecraft_matrices import load_matrices, CONTACT_STAGES
from spacecraft_discretizer import (
    N_MISS, N_STAGES, N_STATES_TOTAL, SINK_STATE, state_index,
)
...
STAGE_LABELS = [f"S{k}\nT-{t:.1f}h" for k, t in
                enumerate([23.4, 9.5, 7.8, 6.2, 2.8, 1.1])]


def solve_from_state(T, O, R, contact_stages, miss_bin, stage):
    """Solve RS-SDA* with point-mass belief on (miss_bin, stage). Returns full_result."""
    init_b = np.zeros(N_STATES_TOTAL)
    init_b[state_index(miss_bin, stage)] = 1.0
