...
Usage:
  ...
  python sdec_spacecraft.py --scenario 2      # partial contact stages
...
from spacecraft_discretizer import (
    ...
    N_MISS, index_to_state, state_index,
    sync_trigger_states,
)

ACT_NAMES = {0: 'WAIT', 1: '+dVT', 2: '-dVT'}
MISS_LABELS = ['[0,1)km COLLISION', '[1,5)km HIGH', '[5,20)km MOD',
               '[20,100)km LOW', '[100,500)km NOM', '[500+)km SAFE']
...
def build_config(exact: bool = False) -> RSSDAConfig:
    if exact:
        return RSSDAConfig(
            maxh=N_STAGES,
            algorithm="exact",
...
def main():
    ...
    if args.scenario == 2:
        contact_stages = [0, 1, 2]
        print("Scenario 2: joint contact only at stages 0-2")
    elif args.scenario == 3:
        print("Scenario 3: fully cooperative (costly comms not yet wired in reward)")
    ...
    config = build_config(args.exact)
    ...
    for s, p in enumerate(init_b):
        if p > 0:
            mb, k = index_to_state(s)
            print(f"  s={s} (miss_bin={mb} {MISS_LABELS[mb]}, stage={k})  p={p:.3f}")
